...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        trunc_length = min(random.randint(4, int(self.block_size*(7/8))), doc_length)
        truncated_doc = document[:trunc_length]

        mask_len = int(random.uniform(-1,1) + trunc_length/4)
        mask_len = np.clip(mask_len, 1, trunc_length - 2)
        start_mask = random.randint(1, trunc_length-mask_len-1)
        prefix = truncated_doc[:start_mask]
        masked_content = truncated_doc[start_mask : start_mask + mask_len]
        suffix = truncated_doc[start_mask+mask_len:]
                        #[prefix] MASK_CHAR [suffix] MASK_CHAR [masked_content] [pads]
